...
- **ReAct 推理循环** — Thought → Action → Observation，支持并行工具调用、执行树管理、死循环检测、Scratchpad 步骤日志防遗忘
...
- **三端交互** — Web UI (React) + TUI (终端) + REST API + WebSocket 实时推送（执行树实时可视化 + 过程日志面板）
...
```bash
npm run dev               # 启动服务（--cwd . 使 workspace 在项目目录下）
