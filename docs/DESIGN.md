...
4. 工具提示词：工具名和描述
5. 记忆提示词：
    1. Session : 当前会话的工作记忆提示词
        1. Hot：保存在内存中，每次 calModel 都需要完整注入的必要提示词
        2. Cold：保存在临时文件中，每步骤结果缓存到本地临时文件，并把位置和描述信息累加到 Hot Session 中，在之后的任务中，通过判断是否需要加载来按需加载，在任务结束后清理
    2. Memory Short：短期记忆，记录所有交互的完整记录，不压缩不总结的，按需加载
    3. Memory Long： 长期记忆，将短期记忆汇总压缩成摘要记忆
6. 自我图式提示词
    1. 身体图式： 对运行自身的计算机的基础资源有一个感知，在任务规划时候不会导致资源使用溢出，让计算机死掉
    2. 灵魂图式：
        1. 环境认知： 世界模型，关于世界的理解，主要描述世界规则、逻辑。约束模型该干什么不该干什么
        2. 自我认知： 知道自己的存在的意义，能力，目的。
    3. 激素：激素指标会影响模型决策
        1. 指标作用描述
        2. 当前激素水平
7. 核心提示词
