...
1. **核心提示词**（core.md）：系统提示词，安全边界、ReAct 核心、内置 tool/skill 描述。不可被用户修改，直接从源码引用
2. **自我图式提示词**（self.md）：
...
3. **工具提示词**（tool.md）：工具名称、id、描述、路径。工具变化时同步更新
4. **技能提示词**（skill.md）：技能名称、id、描述、路径。技能变化时同步更新
5. **Agent 提示词**（agent.md）：Agent 名称、id、描述、路径。Agent 变化时同步更新
6. **记忆提示词**：
...
| 文件 | 用途 | 初始化行为 |
|------|------|-----------|
| `core.md` | 系统提示词：安全边界、ReAct 核心、内置 tool/skill 描述。不可修改 | **直接引用**，不复制 |
| `self.md` | 自我图式模板：身体图式 + 灵魂图式（环境认知/自我认知/激素） | 复制到 workspace |
| `tool.md` | 工具模板：名称、id、描述、路径 | 复制到 workspace |
| `skill.md` | 技能模板：名称、id、描述、路径 | 复制到 workspace |
| `agent.md` | Agent 模板：名称、id、描述、路径 | 复制到 workspace |
| `memory.md` | 长期记忆模板：压缩摘要的基础结构 | 复制到 workspace |

#### workspace 运行时（`workspace/prompts/`）

| 路径 | 类型 | 说明 | qmd 索引 |
|------|------|------|----------|
| `self.md` | 文件 | 自我图式（从模板复制，运行时更新） | 否（量小，直接加载） |
| `tool.md` | 文件 | 工具注册表（名称+id+描述+路径，随工具增长） | 是 |
| `skill.md` | 文件 | 技能注册表（名称+id+描述+路径，随技能增长） | 是 |
| `agent.md` | 文件 | Agent 注册表（名称+id+描述+路径，量小） | 否（直接加载） |
| `memory.md` | 文件 | 长期记忆（压缩摘要，持续累积） | 是 |
| `memory/` | 目录 | 短期记忆（按日期文件 `yyyy-MM-dd.md`，详细交互+Context） | 是 |
